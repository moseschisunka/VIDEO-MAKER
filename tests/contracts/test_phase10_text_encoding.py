...
def test_creation_wizard_uses_authoritative_catalogs_and_fails_closed() -> None:
    ...
    assert 'function projectIsComplete(project)' in library_js
    ...
    assert 'projects.filter(projectIsComplete).length' in library_js
    assert 'filtered = filtered.filter(projectIsComplete);' in library_js
    assert 'Number(project.render_count) > 0' in library_js
    assert 'completed_count >= 5' not in library_js
