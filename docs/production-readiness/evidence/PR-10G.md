...
| Historical Windows SLO rerun | PASS (diagnostic only) | [`PR-10G-slo-windows-after-fast.json`](PR-10G-slo-windows-after-fast.json) — all measured gates pass after bounded FFmpeg preset tuning |
...
| HyperFrames CLI/browser QA | PASS (latest supported clean Ubuntu certification; frozen-RC proof pending) | [`PR-10G-hyperframes-offline-qa.md`](PR-10G-hyperframes-offline-qa.md) — run `34689946142` on `54dfe6b` passed (**2 passed, 2 skipped**); live-provider checks were skipped |
...
| Package data and clean Python smoke | PASS (latest supported CI) | [`PR-1001.md`](PR-1001.md), [`PR-1002.md`](PR-1002.md), and run `34689946142` — clean checkout package-data contract and PR-1002 smoke pass |
| Disposable clean Remotion install/build | PASS (latest supported CI) | [`PR-10G-remotion-clean-build-ci.json`](PR-10G-remotion-clean-build-ci.json) and [`PR-10G-remotion-compositions-ci.txt`](PR-10G-remotion-compositions-ci.txt) — lockfile install, browser ensure, TypeScript, bundle, and 13-composition enumeration pass in run `34689946142` |
| Remotion dependency vulnerability audit | PASS (latest supported CI) | `npm audit --audit-level=high` reports **0 vulnerabilities** in the current lock; clean-install job in run `34689946142` passed the audit gate |
...
## Supported CI feedback (2026-09-12)

Run [`34689946142`](https://github.com/moseschisunka/VIDEO-MAKER/actions/runs/34689946142) validates exact code SHA `54dfe6b0a381ae4b78e6dda456f60dae1da82bba`:
- **Release blockers**: **1,328 passed**, 5 skipped, 1 deselected, 1 warning in 170.52s.
- **Offline regression suite**: **1,789 passed**, 7 skipped, 3 deselected, 1 warning, 1 subtest passed in 263.96s.
- **Clean install and PR-1002 smoke**: passed.
- **Container build, health, and in-image Remotion still render**: passed.
- **Phase 10 SLO/load evidence**: passed.
- **HyperFrames QA**: **2 passed, 2 skipped** in 104.55s.
- **Live-provider checks**: intentionally skipped; this run does not prove external TTS operation or production readiness.
