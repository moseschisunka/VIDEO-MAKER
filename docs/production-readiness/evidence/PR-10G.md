...
| Current local corrective rerun | PASS (Windows diagnostic; supported CI pending) | Checkpoint `6ec8d2f` — the same release-blocker selector plus provider-alias regressions → **1,313 passed, 5 skipped, 1 deselected, 1 warning** in 336.48s; this is local evidence only until the checkpoint is published and CI completes |
| Latest local rerun after evidence-classification hardening | PARTIAL (Windows diagnostic; supported CI pending) | Checkpoint `15d15b8` → **1,313 passed, 5 skipped, 1 deselected, 1 failed** (`PERF-01` warm provider-menu p95 `2.458s` vs `2.0s`) in 468.33s; isolated `test_phase10_slos.py` passes **6/6** in 44.72s, so the local full-suite result is host-load variance and is not supported CI evidence |
| Latest local offline regression | PASS (Windows diagnostic; supported CI pending) | Checkpoint `15d15b8` → `python -m pytest tests -m "not live_provider and not hyperframes_qa" -q` → **1,772 passed, 7 skipped, 3 deselected, 1 warning, 1 subtest passed** in 440.44s; supported Ubuntu CI remains authoritative |
