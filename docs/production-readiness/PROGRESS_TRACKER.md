...
| Last tracker update | Supported push run `33726592541` (commit `6d23cf8`) passed clean install with zero high/critical npm advisories, release-blocker contracts, offline regression, the hardened container/browser matrix, Ubuntu SLO/load/operations evidence, and the new UTF-8 text-integrity contracts; live-provider and HyperFrames opt-ins were skipped; remaining blockers are deployment rollback, external alert delivery/metrics aggregation, trusted-edge security enforcement, and frozen-RC evidence |
...
| `PR-1011` | Preserve UTF-8 authored text across manifest/playbook/config loaders | `PR-1000` | `VERIFIED` | OpenMontage execution agent | [`evidence/PR-10G.md`](evidence/PR-10G.md) — Windows mojibake regression fixed; 3 release-blocking text-integrity tests pass locally and in supported run `33726592541` |
| `PR-10G` | Phase 10 gate | `PR-9G`, `PR-1000`–`PR-1011` | `BLOCKED` | OpenMontage execution agent | [`evidence/PR-10G.md`](evidence/PR-10G.md) — supported run `33726592541` passes the current clean-install/security, offline-contract, container/browser, Ubuntu SLO/load, and text-integrity checkpoint; deployment rollback, external monitoring/alert delivery, trusted-edge enforcement, and frozen-RC evidence remain required |
...
| 2026-09-03 | `PR-10G`/text encoding | Windows' default cp1252 decoding changed authored UTF-8 punctuation (for example `—`) into `â€”` in pipeline/playbook API text | Open manifests, playbooks, generated playbooks, and config with explicit UTF-8; add regression tests and verify the live creation modal | Backend/frontend/QA | Resolved in `6d23cf8`; local **3/3** text-integrity contracts, live Backlot modal, and supported run `33726592541` pass |
