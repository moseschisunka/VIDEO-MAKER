...
| `PR-1015` | Preserve project-relative caption evidence through final review | `PR-1006`, `PR-1011` | `IN_PROGRESS` | OpenMontage execution agent | Newly verified source-footage path defect: final review checks `edit_decisions.subtitles.source` against the caller's working directory instead of the project root; implementation and regression evidence pending |
| `PR-10G` | Phase 10 gate | `PR-9G`, `PR-1000`–`PR-1015` | `BLOCKED` | OpenMontage execution agent | [`evidence/PR-10G.md`](evidence/PR-10G.md) — supported run `33736220396` passes the current clean-install/security, offline-contract, container/browser, Ubuntu SLO/load, and wizard/catalog/library checkpoint; deployment rollback, external monitoring/alert delivery, trusted-edge enforcement, frozen-RC evidence, and PR-1015 integration remain required |
