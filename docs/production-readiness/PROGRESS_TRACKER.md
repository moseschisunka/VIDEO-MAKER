...
| Last tracker update | Supported run `33736220396` (checkpoint `0f3421d`, including source commit `58439f9`) passed all required jobs: clean install with zero high/critical npm advisories, **1057** release-blocker contracts, **1515** offline regression tests, the hardened container/browser matrix, Ubuntu SLO/load/operations evidence, and the UTF-8/catalog/launch/library contracts; live-provider and HyperFrames opt-ins were skipped. Its `PERF-07` p95 was `0.290s`; the immediately preceding run had one transient `0.574s` sample and passed on auditable rerun at `0.280s` with unchanged target/implementation. Remaining blockers are deployment rollback, external alert delivery/metrics aggregation, trusted-edge security enforcement, and frozen-RC evidence |
...
| `PR-10G` | Phase 10 gate | `PR-9G`, `PR-1000`–`PR-1014` | `BLOCKED` | OpenMontage execution agent | [`evidence/PR-10G.md`](evidence/PR-10G.md) — supported run `33736220396` passes the current clean-install/security, offline-contract, container/browser, Ubuntu SLO/load, and wizard/catalog/library checkpoint; deployment rollback, external monitoring/alert delivery, trusted-edge enforcement, and frozen-RC evidence remain required |
