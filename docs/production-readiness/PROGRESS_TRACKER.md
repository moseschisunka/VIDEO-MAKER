...
| Last tracker update | `PR-10G` supported runs `33717170584`/`33718631193` passed clean install, release-blocker contracts, offline regression, the hardened container/browser matrix, Ubuntu SLO/load/operations evidence, and the full HyperFrames render; remaining blockers are deployment rollback, external alert delivery/metrics aggregation, trusted-edge security enforcement, and frozen-RC evidence |
...
| `PR-10G` | Phase 10 gate | `PR-9G`, `PR-1000`–`PR-1010` | `BLOCKED` | OpenMontage execution agent | [`evidence/PR-10G.md`](evidence/PR-10G.md) — supported runs `33717170584`/`33718631193` pass clean install, offline contracts, container/browser, Ubuntu SLO/load, fake drills, and HyperFrames render; deployment rollback, external monitoring/alert delivery, trusted-edge enforcement, and frozen-RC evidence remain required |
